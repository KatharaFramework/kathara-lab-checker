...
class HTTPCheck(AbstractCheck):
    ...
    def check(self, device_name: str, test_params: dict) -> list[CheckResult]:
        ...
        # For GET, typically just "curl -s -o /dev/null -w '%{http_code}' <url>"
        # For POST or custom methods, we can add "-X <METHOD>"
        curl_cmd = f"curl -s -X {method} -o /tmp/curl_body.txt -w '%{{http_code}}' '{url}'"
        ...
        output = get_output(exec_output).strip()

        expected_code = test_params.get("status_code", 200)
        if not output.isdigit():
            results.append(CheckResult(desc, False, f"curl output not a digit: '{output}'"))
        else:
            actual_code = int(output)
        ...
        if "regex_body" in test_params or "body_contains" in test_params:
            cat_cmd = "cat /tmp/curl_body.txt"
            try:
                body_exec_output = self.kathara_manager.exec(
                    machine_name=device_name,
                    command=cat_cmd,
                    lab_hash=self.lab.hash
                )
                body = get_output(body_exec_output)
            except MachineNotRunningError as e:
                body = ""
