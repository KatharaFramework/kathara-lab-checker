...
class BridgeCheck(AbstractCheck):
    ...
    def check(self, device_name: str, bridges_configuration: list, lab: Lab) -> CheckResult:
        ...
        try:
            output = get_output(
                kathara_manager.exec(
                    machine_name=device_name, lab_hash=lab.hash, command="ip -d -j link show type bridge"
                )
            )
        ...
        if len(output) != len(bridges_configuration):
            reason = f"Device {device_name} has {len(output)} bridges running while configuration asserts {len(bridges_configuration)} bridges running"
            return CheckResult(self.description, False, reason)
        ...
        current_conf_bridge = bridges_configuration[0]
            ...
            if curr_conf:
                ...
                if "vlan_tags" in curr_conf:
                    ...
                    if configued_vlan_set == vlan_numbers_set:
                        ...
                        reason = f"Device {device_name} has bridge {current_running_bridge['ifname']} with errors in vlans {symmetric_difference}"
                        return CheckResult(self.description, False, reason)
                else:
                    ...
                    if curr_conf["pvid"] == vlan_pvid:
                        ...
                        reason = f"Device {device_name} has bridge {current_running_bridge['ifname']} with errors in untagged ifaces of vlans"
                        return CheckResult(self.description, False, reason)
            else:
                reason = (
                    f"Device {device_name} has bridge {current_running_bridge['ifname']} with errors in vlans"
                )
                return CheckResult(self.description, False, reason)
    ...
    def run(self, devices_to_daemons: dict[str, list[str]], lab: Lab) -> list[CheckResult]:
        ...
        for device_name, list_bridges in devices_to_daemons.items():
            self.logger.info(f"Checking bridge configuration on `{device_name}`...")
